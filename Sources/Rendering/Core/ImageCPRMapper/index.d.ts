...
export interface vtkImageCPRMapper extends vtkAbstractMapper3D {
	 ...
	 * If no such point data is specified, a uniform direction can be used instead ( @see getUniformDirection @see getUseUniformDirection )
	 ...
	 * @param imageData
	 */
	setImageDataData(imageData: vtkImageData): void;

	/**
	 * Set the connection for the volume
	 * You can also use `publicAPI.setInputConnection(imageData, 0);`
	 * @see setImageDataData
	 * @param imageData
	 */
	setImageDataConnection(imageData: vtkOutputPort): void;
...
/**
 * CPR in vtkImageCPRMapper stands for Curved Planar Reformation. This
 * specialised mapper takes as input a vtkImageData representing a volume
 * ( @see setImageDataData ) and a vtkPolyData representing a centerline
 * ( @see setCenterlineData ). The mapper also need to have a direction per
 * point or for all points, in a form of a vec3. This can be specified using
 * a uniform direction ( @see getUniformDirection @see getUseUniformDirection
 * ) or a point data array ( @see setDirectionArrayName
 * @see setDirectionArrayOffset ). Every point, vector or length specified to
 * the mapper (centerline points, direction vectors, width...) use model
 * coordinates of the volume used as input ( @see setImageDataData ).
 *
