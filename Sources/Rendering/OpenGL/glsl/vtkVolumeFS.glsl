...
vec3 vecISToVC(vec3 dirIS) {
  return volume.vecISToVCMatrix * (dirIS / vec3(512.0, 512.0, 368.0));
}
...
vec4 computeDensityNormal(vec3 opacityUCoords[2], float opacityTextureHeight,
  ...
  opacityG.xyz = vecISToVC(opacityG.xyz);
...
vec4 computeNormalForDensity(vec3 posIS, out vec3 scalarInterp[2],
  ...
  vec4 result;
  result.xyz =
      (scalarInterp[0] - scalarInterp[1]) * volume.inverseSpacing;
  result.xyz = vecISToVC(result.xyz);
